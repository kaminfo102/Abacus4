...
class Rod {
  ...
  constructor(position: number, beads: Bead[], value: number) {
  ...
  drawRod(context: CanvasRenderingContext2D): void {
    ...
    context.beginPath();
    context.moveTo(this.evalXPos(), TOP_MARGIN + NUMBER_HEIGHT);
    context.lineTo(this.evalXPos(), TOP_MARGIN + NUMBER_HEIGHT + HEIGHT);
    context.stroke();
    context.restore();
  }
...
export class Abacus {
  ...
  drawFrame(): void {
    ...
    this.context.lineWidth = FRAME_LINE_WIDTH;
    ...
    this.context.rect(LEFT_MARGIN, TOP_MARGIN + NUMBER_HEIGHT, this.width, HEIGHT);
    this.context.moveTo(LEFT_MARGIN + FRAME_LINE_WIDTH / 2, TOP_MARGIN + NUMBER_HEIGHT + HEAVEN);
    this.context.lineTo(LEFT_MARGIN + this.width - FRAME_LINE_WIDTH / 2, TOP_MARGIN + NUMBER_HEIGHT + HEAVEN);
    this.context.stroke();
    ...
    for (let i = 0, x = LEFT_MARGIN + DISTANCE_RODS; i < this.numberOfRods; ++i, x += DISTANCE_RODS) {
      if ((i - middle) % 3 === 0) {
        this.context.beginPath();
        this.context.arc(x, TOP_MARGIN + NUMBER_HEIGHT + HEAVEN, DOT_SIZE, 0, Math.PI * 2, false);
        this.context.fill();
        this.context.stroke();
      }
    }
